...
이로 인해 HTTP/0.9에서 다양한 타입의 정보를 전송할 수 있게 되었고, 헤더와 상태 코드로 인해 클라이언트와 서버 간의 통신이 원활해졌습니다. 하지만 HTTP/1.0은 각 요청마다 계속 TCP 연결을 열고, 응답하면 닫았어야 했으며, TCP가 연결될 때 슬로우 스타트가 발생하여 속도면에서 문제가 많았습니다. 인터넷이 발달함에 있어 HTTP/1.0으로는 웹에서 효율적으로 작동하기 어렵다는 것이 HTTP/1.1이 개발된 이유 중 하나입니다.
